...
void nelderMead(int dimension, int protein_length, float start[], float xmin[]){

	const float step = 1.0f;
	const int n = dimension;

	thrust::device_vector<float> simplex(n * (n + 1));
	float * p_simplex = thrust::raw_pointer_cast(&simplex[0]);

	nelderMead_initialize(dimension, p_simplex);
